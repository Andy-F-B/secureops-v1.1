...
import { WhitelabelConfig, PortalRequests, CustomerBilling, SocialAnalytics } from "@/api/supabaseClient";
...
export default function DevPanel() {
  ...
  const loadPortalRequests = async () => {
    const requests = await PortalRequests.list();
    setPortalRequests(requests);
  };

  const loadBillingData = async () => {
    const data = await CustomerBilling.list();
    setBillingData(data);
  };

  const loadSocialAnalytics = async () => {
    const data = await SocialAnalytics.list();
    if (data.length > 0) setSocialAnalytics(data[0]);
  };

  const loadConfigs = async () => {
    setLoading(true);
    const all = await WhitelabelConfig.list();
    setConfigs(all);
    setLoading(false);
  };
  ...
  const deleteConfig = async (cfg) => {
    ...
    await WhitelabelConfig.delete(cfg.id);
    await loadConfigs();
  ...
  const resetConfig = async (cfg) => {
    ...
    await WhitelabelConfig.update(cfg.id, {
  ...
  const approveRequest = async (request) => {
    ...
    await PortalRequests.update(request.id, { status: "approved" });
    await WhitelabelConfig.create({
      code,
  ...
  const denyRequest = async (request) => {
    setRequestsLoading(true);
    await PortalRequests.update(request.id, { status: "denied" });
    await loadPortalRequests();
    setRequestsLoading(false);
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => { loadConfigs(); loadPortalRequests(); loadBillingData(); loadSocialAnalytics(); }}
              className="flex items-center gap-2 text-gray-400 hover:text-white text-sm bg-gray-800 border border-gray-700 px-4 py-2 rounded-xl"
            >
              <RefreshCw className="w-4 h-4" /> Refresh
            </button>
            <button
              onClick={() => { sessionStorage.removeItem("secureops_dev"); window.location.href = createPageUrl("Home"); }}
        ...
        <div className="flex gap-2 mb-8 border-b border-gray-800">
          {[
            ...
            { key: "requests", label: "Requests", badge: portalRequests.filter(r => r.status === "pending").length },
            ...
            <button
              ...
              className={`px-4 py-3 font-semibold border-b-2 transition-colors flex items-center gap-2 ${
                activeTab === tab.key ? "border-orange-500 text-white" : "border-transparent text-gray-500 hover:text-gray-300"
              }`}
            >
              {tab.key === "requests" && <Bell className="w-4 h-4" />}
              {tab.label}
              {tab.badge > 0 && (
          ...
          <>
            <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 mb-8">
              ...
              <p className="text-gray-500 text-xs mb-4">
                Create a code that a customer can use at <span className="text-gray-300">/WhitelabelSetup</span> to configure their portal.
              </p>
                  ...
                  <div key={cfg.id} className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                    <div className="flex items-center justify-between p-5">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-black font-mono text-sm"
                          style={{ background: cfg.theme_hex || "#2563eb" }}>
                          {cfg.code}
                        </div>
                        <div>
                          <p className="text-white font-semibold">
                            {cfg.company_name || <span className="text-gray-500 italic">No company name</span>}
                          </p>
                          <p className="text-gray-500 text-xs">
                            ...
                            {" "}• Created {cfg.created_at ? new Date(cfg.created_at).toLocaleDateString() : "unknown"}
                          </p>
                        </div>
                      </div>
                    ...
                    {expanded[cfg.id] && (
                      <ConfigDetail
                        ...
                        setForm={(updates) => setAdminForms(prev => ({
                          ...prev,
                          [cfg.id]: { ...(adminForms[cfg.id] || {}), ...updates }
                        }))}
            ...
            <div className="mt-8 bg-orange-900/20 border border-orange-800/40 rounded-2xl p-4 flex items-start gap-3">
              ...
              <p className="text-orange-300 text-sm">
                Deleting a configuration removes all setup data for that code. Officers, sites, and records created in the main portal are <strong>not</strong> automatically deleted. To fully wipe a tenant's data, delete their officers and sites manually.
              </p>
            </div>
          </>
        )}
                ...
                <div key={req.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm mt-2">
                        Plan: <span className="text-blue-400">
                          {req.plan === "1-50" ? "Startup (1-50)" :
                           req.plan === "51-200" ? "Growth (51-200)" :
                           req.plan === "200+" ? "Enterprise (200+)" : "Whitelabel"}
                        </span>
                      </p>
                    </div>
                  ...
                  <div className="flex gap-2 justify-end">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs bg-red-900/30 hover:bg-red-900/50 text-red-400 border border-red-800/50 px-4 py-2 rounded-xl disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={requestsLoading}
                      className="flex items-center gap-1.5 text-xs bg-green-900/30 hover:bg-green-900/50 text-green-400 border border-green-800/50 px-4 py-2 rounded-xl disabled:opacity-50"
                    >
                  ...
                  <div key={bill.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <span className={`text-xs px-3 py-1 rounded-full ${
                        bill.status === "paid" ? "bg-green-900/30 text-green-400" :
                        bill.status === "late" ? "bg-red-900/30 text-red-400" :
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="text-gray-400 text-xs">
                          {bill.last_payment_date ? new Date(bill.last_payment_date).toLocaleDateString() : "—"}
                        </p>
                      </div>
                      <div>
                        <p className="text-gray-500 text-xs">Next Charge</p>
                        <p className="text-gray-400 text-xs">
                          {bill.next_charge_date ? new Date(bill.next_charge_date).toLocaleDateString() : "—"}
                        </p>
                      </div>
                    </div>
                  </div>
